...
// Enhanced interfaces for rename functionality
...
export class OpenSCADRenameProvider implements monaco.languages.RenameProvider {
  ...
  async prepareRename(
    ...
    try {
      ...
      throw new Error('Symbol location not available');
  ...
  private async findAllReferences(
    ...
    try {
      ...
      const references = this.symbolProvider.findSymbolReferences(ast, symbol.name);
      const renameLocations = references
        .filter(ref => ref.loc)
        .map(ref => ({
          range: new monaco.Range(
  ...
  /**
   * Get AST from parser service (placeholder implementation)
   */
  private getASTFromParserService(): ASTNode[] | null {
    // TODO: Implement actual AST retrieval from parser service
    // This is a placeholder that would be replaced with real parser integration
    return null;
  }
  ...
  private calculateOffset(model: monaco.editor.ITextModel, position: monaco.Position): number {
    ...
    for (let line = 1; line < position.lineNumber; line++) {
      offset += model.getLineLength(line) + 1; // +1 for newline
    }
