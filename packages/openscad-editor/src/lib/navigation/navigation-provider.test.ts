...
describe('OpenSCADNavigationProvider', () => {
  ...
  describe('provideReferences', () => {
    it('should find all references to a symbol', async () => {
      // Position on "size" definition
      const position = new monaco.Position(13, 1);
  ...
  describe('searchSymbols', () => {
    ...
    it('should support fuzzy matching', async () => {
      const symbols = await navigationProvider.searchSymbols('cvh', { fuzzy: true });
      
      expect(symbols).toBeDefined();
      // Should find "cube_with_hole" with fuzzy matching
