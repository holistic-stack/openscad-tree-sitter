...
module tiny() {
  cube(1);
    ...
    it('should remove overlapping ranges', async () => {
      const code = `
module outer() {
  module inner() {
    cube([10, 10, 10]);
  ...
  inner();
      ...
      // Should handle nested modules without overlapping ranges
      expect(ranges.length).toBeGreaterThanOrEqual(1);
      ...
      for (let i = 0; i < ranges.length - 1; i++) {
        for (let j = i + 1; j < ranges.length; j++) {
          ...
          // Ranges should not completely overlap
          const completeOverlap = (
            (range1.start <= range2.start && range1.end >= range2.end) ||
            (range2.start <= range1.start && range2.end >= range1.end)
          );
          
          expect(completeOverlap).toBe(false);
        }
      }
    });
  });
  ...
  describe('Factory Function', () => {
    it('should create folding provider with default config', () => {
    ...
    it('should create folding provider with custom config', () => {
      const { createFoldingProvider } = require('./folding-provider');
