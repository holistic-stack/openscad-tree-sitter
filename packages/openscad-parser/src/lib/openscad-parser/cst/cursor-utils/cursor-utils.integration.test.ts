  ...
  describe('Transformations', () => {
    it('should handle translate transform', () => {
      ...
      const simplifiedRootNode = tree?.rootNode ? { 
        ...
        text: tree.rootNode.text?.substring(0, 70) + (tree.rootNode.text && tree.rootNode.text.length > 70 ? '...' : '') 
      } : null;

      expect(tree).not.toBeNull();

      const logFilePath = path.resolve(__dirname, '../../../../../diagnostic_log.txt');
      try { fs.writeFileSync(logFilePath, ''); } catch (e) { /* ensure file is clear or created */ }
      ...
      expect(cursor.gotoFirstChild()).toBe(true); 
      log(`Cursor at: ${cursor.nodeType} (expected accessor_expression for translate)`);
      ...
      while(cursor.nodeType !== 'array_literal' && currentDepth < maxDescendDepth) {
