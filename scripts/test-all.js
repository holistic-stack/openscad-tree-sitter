...
async function main() {
  ...
  // Step 3: Run the Node.js tests with node:test
  if (!runCommand('node --test test/nodejs/*.js', 'Run Node.js tests')) {
    process.exit(1);
  }
  ...
  if (!runCommand('node --test test/nodejs/test-wasm-build.js', 'Test WebAssembly build')) {
    process.exit(1);
  }
