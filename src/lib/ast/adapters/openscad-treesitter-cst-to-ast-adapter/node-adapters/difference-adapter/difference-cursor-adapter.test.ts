/**
 * Tests for DifferenceOperation cursor adapter
 * 
 * Following Test-Driven Development (TDD) principles, this file contains
 * tests for the DifferenceOperation cursor adapter before its implementation.
 */

import { describe, it, expect, beforeEach } from 'vitest';
import { TreeSitterNode, TreeCursor } from '../../../../types/cst-types';
import { DifferenceOperation } from '../../../../types/openscad-ast-types';
import { differenceCursorAdapter } from './difference-cursor-adapter';

describe('DifferenceOperation Cursor Adapter', () => {
  // Create child nodes to serve as children in the difference operation
  ...
  // Test utility for creating a difference node
  function createTestDifferenceNode(): TreeSitterNode {
    // Create identifier node for 'difference'
    ...
    // Create arguments node (empty for difference)
    const argumentsNode = {
      type: 'arguments',
    ...
    // Create body block for difference operation
    const bodyNode = {
      type: 'block',
    ...
    // Create difference operation node
  ...
  // Test utility for creating a cursor pointing to a difference node
  ...
  let differenceNode: TreeSitterNode;
  let cursor: TreeCursor;
  
  beforeEach(() => {
    differenceNode = createTestDifferenceNode();
    cursor = createTestCursor(differenceNode);
  });
  
  it('should convert a difference node to a DifferenceOperation AST node', () => {
    // Act
    const result = differenceCursorAdapter(cursor);
    
    // Assert
    expect(result).toBeDefined();
    expect(result.type).toBe('DifferenceOperation');
    
    // Should have the correct position information
    expect(result.position).toEqual({
      startLine: differenceNode.startPosition.row,
      startColumn: differenceNode.startPosition.column,
      endLine: differenceNode.endPosition.row,
      endColumn: differenceNode.endPosition.column
    });
  ...
  it('should preserve the order of children', () => {
    ...
    // Assert
    // First child should be cube (the base object), second should be sphere (the object to subtract)
  ...
  it('should follow the OpenSCAD difference operation semantics', () => {
    // Act
    const result = differenceCursorAdapter(cursor);
    
    // Assert - in OpenSCAD, the difference operation semantics specify that
    // the first child is the base object, and remaining children are subtracted from it
    expect(result.children.length).toBe(2);
    expect(result.children[0].position.startLine).toBe(1); // First child (base) is the cube on row 1
    expect(result.children[1].position.startLine).toBe(2); // Second child (subtracted) is the sphere on row 2
    
    // Note: In the interface, we represent all children in the 'children' array
    // rather than separating them into 'base' and 'subtracted' properties
  });
});
